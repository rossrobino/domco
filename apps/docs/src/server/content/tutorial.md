...
To get started, you'll need to have [Node](https://nodejs.org), [Bun](https://bun.sh/), or [Deno](https://deno.com) or installed on your computer. Then run the `create` script below to create a new project. If you already have an existing client-side Vite project you would like to add a server to, check out the [examples repository](https://github.com/rossrobino/domco-examples).
...
Each `+script.(js,ts,jsx,tsx)` file within `src/client/` will be [processed as an entry point](https://rollupjs.org/configuration-options/#input) by Vite. Client-side scripts can be used on the server _without_ a page by using the [`client:script`](#client%3Ascript) virtual module.
...
Each `+style.css` file within `src/client/` also becomes an entry point. Styles can be accessed on the server using the [`client:style`](#client%3Astyle) virtual module.
...
domco takes care of these problems using [virtual modules](https://vitejs.dev/guide/api-plugin.html#virtual-modules-convention). You can easily serve a `+page` or include the tags for a `+script` or `+style` in a response. domco ensures the correct assets are linked during development and in production.
...
```ts {3,10}
// src/server/+app
// `html` is transformed content of `src/client/+page.html`
import { html } from "client:page";
// `src/client/other/+page.html`
import * as other from "client:page/other";
...
`client:style` holds the asset information for a `+style.css` entrypoint.

```ts
// /client/+style.css
import * as style from "client:style";
// /client/other/+style.css
import * as other from "client:style/other";
